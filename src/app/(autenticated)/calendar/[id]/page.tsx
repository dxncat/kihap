...
import { getRankById, getSessionById, getStudentsByRankId, getStudentsWithoutAttendance } from "@/actions"
...
export default async function AttendancePage({ params, searchParams }: Props) {
    ...
    const allStudentsWithoutAttendance = await getStudentsWithoutAttendance({ sessionId: id, rankId: session_to.rankId })
                ...
                <div className="mb-8">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        ...
                        <div className="flex items-center space-x-4">
                            {rankInfo && (
                                <Badge className={`${getBeltColor(rankInfo.name)} px-3 py-1`}>
                                    {rankInfo.name}
                                </Badge>
                            )}
                                ...
                                <Table>
                                    ...
                                    <TableBody>
                                        {filteredStudents.map(async (student) => {
                                            ...
                                            const nextRank = await getRankById({ rankId: student.Rank.id + 1 })

                                            const progressPercentage = Math.min(
                                                (student.currentHours / nextRank.hoursRequired) * 100,
                                                100,
                                            )
                                                ...
                                                <TableRow key={student.id} className="border-white/10 hover:bg-white/5 transition-colors">
                                                    ...
                                                    <TableCell>
                                                        <div className="text-white/80">
                                                            {student.currentHours}h / {nextRank.hoursRequired}h
                                                        </div>
                                                    </TableCell>
                                                    <TableCell>
